...
function love.update(dt)
  ...
  if love.mouse.isDown(1) then
	local printx = love.mouse.getX()	--x coordinate of the mouse
	local printy = love.mouse.getY()	--y coordinate of the mouse
	line = {}
	...
	line.fixture = love.physics.newFixture(line.body, line.shape, 5) --Fixes the object to the world, otherwise the line-rider would just fall through it. Density is 5.
		--We keep a table of all the line parts we have. We can then draw and manipulate them later
...
--	love.graphics.polygon("fill", line.body:getWorldPoints(line.shape:getPoints()))
	...
	if oldBody ~= nil then
		...
		joint = love.physics.newDistanceJoint(line.body, oldBody, line.x1, line.y1, line.x2, line.y2, true)
		
		--line.body = love.physics.newBody(world, printx, printy, "static")
		--line.shape = love.physics.newEdgeShape(oldx, oldy, printx, printy)
		--line.fixture = love.physics.newFixture(line.body, line.shape, 5)
		table.insert(objects.lines, joint)
		print("yolo")
	end
	table.insert(objects.lineConnects, line)
...
--  love.graphics.polygon("fill", objects.block2.body:getWorldPoints(objects.block2.shape:getPoints()))
  ...
  for _, line in pairs(objects.lines) do
	--love.graphics.line("fill", block.body:getX1() - Camera.x, block.body:getY1() - Camera.y, 0, 0, 5, 5) --:getWorldPoints(block.shape:getPoints()))
  end
