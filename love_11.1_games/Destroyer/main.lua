...
explosionList = {"sounds/cannon.wav", "sounds/explosion2.wav", "sounds/explosion3.wav", "sounds/explosion4.wav"}
			...
			if checkValues(fixture1:getUserData(), fixture2:getUserData(), "ball") then
				...
				if options.audio.sfx then
					if SensorsDestroyed == #Sensor - 1 and options.graphics.slowmotion then
						TEsound.play(explosionList, "cannon", 1, 0.1)
						TEsound.play(explosionList, "cannon", 1, 0.1)
						TEsound.play(explosionList, "cannon", 1, 0.1)
						TEsound.play(explosionList, "cannon", 1, 0.1)
						TEsound.play(explosionList, "cannon", 1, 0.1)
						...
						TEsound.play(explosionList, "cannon", 1, 0.4)
						TEsound.play(explosionList, "cannon", 1, 0.4)
						TEsound.play(explosionList, "cannon", 1, 0.4)
						TEsound.play(explosionList, "cannon", 1, 0.4)
						TEsound.play(explosionList, "cannon", 1, 0.4)
...
function love.load()
	...
	imageData = love.image.newImageData( icon:getWidth(), icon:getHeight() )
...
--	if love.filesystem.exists("completed.lua") and demo then
	if love.filesystem.getInfo("completed.lua") and demo then
...
--	while not love.filesystem.exists("levels/"..currentPack.."/level"..currentLevel..".lua") do
	while not love.filesystem.getInfo("levels/"..currentPack.."/level"..currentLevel..".lua") do
	...
	if temp_inv == "true" then temp_inv = "no" else temp_inv = "no" end
	...
	if temp_vignette == "true" then temp_vignette = "disabled" else temp_vignette = "disabled" end
	temp_grain = tostring(options.graphics.shader)
	if temp_grain == "true" then temp_grain = "disabled" else temp_grain = "disabled" end
	temp_god = tostring(options.cheats.SensorsAreFtw)
	if temp_god == "true" then temp_god = "disabled" else temp_god = "disabled" end
...
--	po2Supported = love.graphics.isSupported("npot")
	po2Supported = love.graphics.getSupported().npot
--	subtractiveSupported = love.graphics.isSupported("subtractive")
	subtractSupported = love.graphics.getSupported().subtractive
--	canvasSupported = love.graphics.isSupported("canvas")
	canvasSupported = love.graphics.getSupported().canvas
	...
	camera:newLayer(1, function()
		...
		for i,v in ipairs(DeathParticle) do
			love.graphics.setColor(0.792,0.561,0.33,DeathParticleAlpha[1])
			love.graphics.polygon("fill", v.body:getWorldPoints(v.shape:getPoints()))	
		end
...
--		love.graphics.drawq(RedTiles, v.quad, v.x, v.y)
		love.graphics.draw(RedTiles, v.quad, v.x,v.y)
	end
end
...
--			love.graphics.drawq(GreyTiles, v.quad, v.x, v.y)
			love.graphics.draw(GreyTiles, v.quad, v.x,v.y)
		end
	end
end
...
--		print("testaiming")
		...
		if explode then			
			...
			if options.graphics.particleEffects == true then
				...
				for i,v in ipairs(Particle) do
					--if not v.isDestroyed then
						v.body:applyLinearImpulse(math.random(-400,400),math.random(-400,400))
					--end
					--if options.cheats.colorfulExplosion then
						v.r, v.g, v.b = math.random(),math.random(),math.random() --Colorful Explosions / Set a boolean variable for "original mappack completed" if true activate this (maybe)
					--else
					--	v.r, v.g, v.b = 0.274,0.274,0.274
					--end
				end
			end		
...
--		love.graphics.setBlendMode("multiplicative")
		love.graphics.setBlendMode("alphamultiply")
...
function menu:callback(cb)
	...
	elseif cb == "fs" then	
		...
		love.load()
	elseif cb == "res" then
		love.window.setMode( videomodes[currentmode].width, videomodes[currentmode].height )
