...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	objects.rider.body = love.physics.newBody(world, 650/2, 0, "dynamic") --Determines where the object will start. In the center of the world and dynamic(moves around)
	...
	objects.rider.shape = love.physics.newRectangleShape(objects.rider.width, objects.rider.height) --the ball's shape has a radius of 20
	-- Create a fixture for the rider
	objects.rider.fixture = love.physics.newFixture(objects.rider.body, objects.rider.shape) -- Attach fixture to body and give it a density of 1.
	...
	objects.rider.img = love.graphics.newImage("images/tobboggan.png")
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	
	--initial graphics setup
	love.graphics.setBackgroundColor(0.41, 0.53, 0.97) --set the background color to blue
	love.window.setMode(1300, 1000) --set the window dimensions to 1700 by 1000
end
